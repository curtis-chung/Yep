...
const YelpStars = ({ currBizRating, height, width }) => {
    // console.log("currBizRating", currBizRating)
    // console.log(typeof (currBizRating))

    const bizRating = parseFloat((Math.round(currBizRating * 2) / 2).toFixed(1));
    ...
    // console.log(bizRating, typeof (bizRating), className[bizRating])

    return (
        <svg id="rating" class={`rating ${className[bizRating]}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 865 145" width={width} height={height}>
            <defs>
